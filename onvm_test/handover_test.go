...
import (
	...
	// "github.com/nycu-ucr/onvmpoller"  // Only needed for TestMultiN2HandoverConcurrent
...
func TestN2Handover(t *testing.T) {
	...
	HandoverLogger.Infof("type of conn = %s", reflect.TypeOf(conn))
	assert.Nil(t, err)
	if err == nil {
	...
	*/
	...
	mobileIdentity5GS := nasType.MobileIdentity5GS{
		Len:    13,                                                                                    // suci
		Buffer: []uint8{0x01, 0x02, 0xf8, 0x39, 0xf0, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x47, 0x78}, // 7487
	}
	...
	amfUeNgapId := test.GetAmfUeNgapId(ue, ngapMsg.InitiatingMessage.Value.DownlinkNASTransport)
	if amfUeNgapId != nil {
		ue.AmfUeNgapId = amfUeNgapId.Value
		HandoverLogger.Infof("Updated AmfUeNgapId to: %d", ue.AmfUeNgapId)
	}
	HandoverLogger.Infof("Sending Authentication Response with AmfUeNgapId: %d, RanUeNgapId: %d", ue.AmfUeNgapId, ue.RanUeNgapId)
	...
	*/
	...
	handoverRequest, err := ngap.Decoder(recvMsg[:n])
	assert.Nil(t, err)

	// Extract AmfUeNgapId from Handover Request
	var targetAmfUeNgapId int64
	if handoverRequest.Present == ngapType.NGAPPDUPresentInitiatingMessage &&
		handoverRequest.InitiatingMessage.Value.Present == ngapType.InitiatingMessagePresentHandoverRequest {
		for _, ie := range handoverRequest.InitiatingMessage.Value.HandoverRequest.ProtocolIEs.List {
			if ie.Id.Value == ngapType.ProtocolIEIDAMFUENGAPID {
				targetAmfUeNgapId = ie.Value.AMFUENGAPID.Value
				HandoverLogger.Infof("Extracted AmfUeNgapId from Handover Request: %d", targetAmfUeNgapId)
				break
			}
		}
	}

	if targetAmfUeNgapId == 0 {
		t.Fatalf("Failed to extract AmfUeNgapId from Handover Request")
	}
...
func TestMultiN2HandoverConcurrent(t *testing.T) {
	...
	// onvmpoller.SetLocalAddress(testUsedIpAddr)  // Commented out - uncomment if using ONVM
