...
import (
	...
	// "github.com/nycu-ucr/onvmpoller"  // Commented out - only needed for ONVM/DPDK tests
...
func TestPaging(t *testing.T) {
	...
	// onvmpoller.SetLocalAddress(testUsedIpAddr)  // Commented out - only needed for ONVM/DPDK tests
	 ...
	 */
	go func() {
		// RAN connect to UPF (optional - for receiving downlink traffic)
		upfConn, err := test.ConnectToUpf(ranN3Ipv4Addr, "127.0.0.8", 2152, 2152)
		if err != nil {
			PagingLogger.Warnf("Could not connect to UPF at 127.0.0.8:2152: %v", err)
			PagingLogger.Warnln("This is optional for L25GC mode - continuing without UPF connection")
			return
		}
		defer upfConn.Close()
	...
	if exec_type == "free5GC" {
		...
		if _, err := os.Stat("./python_paging_client.py"); os.IsNotExist(err) {
			PagingLogger.Warnln("python_paging_client.py not found - skipping downlink traffic injection")
			PagingLogger.Warnln("You may need to manually trigger downlink traffic or the test will timeout")
		} else {
			cmd := exec.Command("python3", "./python_paging_client.py")
			_, err = cmd.Output()
			if err != nil {
				PagingLogger.Warnf("Instruct DN to send downlink traffic Error: %v\n", err)
				PagingLogger.Warnln("Continuing without downlink traffic injection")
			}
		}
	} else if exec_type == "XIO-free5GC" {
		/* XIO-L25GC version with 1 host */
		// onvmpoller.TriggerPaging(upfServiceId, testUsedIpAddr, "10.60.0.1")  // Commented out
		PagingLogger.Warnln("XIO-free5GC mode requires onvmpoller - skipping")
	} else {
...
func TestTriggerPaging(t *testing.T) {
	// onvmpoller.SetLocalAddress(testUsedIpAddr)  // Commented out - only needed for ONVM/DPDK tests
	// onvmpoller.TriggerPaging(upfServiceId, testUsedIpAddr, "60.60.0.1")  // Commented out
	t.Skip("This test requires ONVM/DPDK setup")
}
