...
import (
	...
	"github.com/nycu-ucr/CommonConsumerTestData/UDR/TestRegistrationProcedure"
...
func GetAuthSubscription(k, opc, op string) models.AuthenticationSubscription {
	...
	// Note: SequenceNumber is a string in older nycu-ucr/openapi versions
	// The UDM will handle conversion to proper format
...
func NewRanUeContext(supi string, ranUeNgapId int64, cipheringAlg, integrityAlg uint8, anType ...models.AccessType) *RanUeContext {
	ue := RanUeContext{}
	ue.RanUeNgapId = ranUeNgapId
	ue.Supi = supi
	ue.CipheringAlg = cipheringAlg
	ue.IntegrityAlg = integrityAlg
	// Default to 3GPP access if not specified (backward compatible)
	if len(anType) > 0 {
		ue.AnType = anType[0]
	} else {
		ue.AnType = models.AccessType__3_GPP_ACCESS
	}
	return &ue
}
