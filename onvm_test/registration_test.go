...
func TestRegistration(t *testing.T) {
	...
	// New UE
	// Use IMSI that matches the SUCI encoding: imsi-2089300007487
	ue := test.NewRanUeContext("imsi-208930000007488", 1, security.AlgCiphering128NEA0, security.AlgIntegrity128NIA2)
	ue.AmfUeNgapId = 1
	...
	// SUCI encoding for IMSI: imsi-2089300007487 (MCC=208, MNC=93, MSIN=00007487)
	// The SUCI will be: suci-0-208-93-0-0-7487
	mobileIdentity5GS := nasType.MobileIdentity5GS{
		Len: 13, // suci
	...
	// Debug: Print actual message type and rejection cause if applicable
	msgType := nasPdu.GmmHeader.GetMessageType()
	fmt.Printf("[DEBUG] Received NAS message type: 0x%02x\n", msgType)
	fmt.Printf("[DEBUG] Expected Authentication Request: 0x%02x\n", nas.MsgTypeAuthenticationRequest)
	fmt.Printf("[DEBUG] Registration Reject type: 0x%02x\n", nas.MsgTypeRegistrationReject)
	if msgType == nas.MsgTypeRegistrationReject {
		fmt.Printf("[DEBUG] Registration Reject! Cause: 0x%02x\n", nasPdu.RegistrationReject.Cause5GMM.GetCauseValue())
	}
	...
	// Update AmfUeNgapId from the received message
	amfUeNgapId := test.GetAmfUeNgapId(ue, ngapPdu.InitiatingMessage.Value.DownlinkNASTransport)
	if amfUeNgapId != nil {
		ue.AmfUeNgapId = amfUeNgapId.Value
		fmt.Printf("[DEBUG] Updated AmfUeNgapId to: %d\n", ue.AmfUeNgapId)
	}
	fmt.Printf("[DEBUG] Sending Authentication Response with AmfUeNgapId: %d, RanUeNgapId: %d\n", ue.AmfUeNgapId, ue.RanUeNgapId)
	//
	...
	// Debug: Check what NGAP message type we received
	fmt.Printf("[DEBUG] NGAP PDU Present: %v\n", ngapPdu.Present)
	if ngapPdu.Present == ngapType.NGAPPDUPresentInitiatingMessage {
		fmt.Printf("[DEBUG] NGAP Procedure Code: %v\n", ngapPdu.InitiatingMessage.ProcedureCode.Value)
		if ngapPdu.InitiatingMessage.ProcedureCode.Value == ngapType.ProcedureCodeDownlinkNASTransport {
			fmt.Printf("[DEBUG] Received DownlinkNASTransport\n")
		} else if ngapPdu.InitiatingMessage.ProcedureCode.Value == ngapType.ProcedureCodeErrorIndication {
			fmt.Printf("[DEBUG] Received Error Indication! This means AMF rejected the Authentication Response\n")
			t.Fatalf("AMF sent Error Indication - check Authentication Response format")
		}
	}

	require.Equal(t, ngapPdu.Present, ngapType.NGAPPDUPresentInitiatingMessage, "Expected InitiatingMessage")
	require.Equal(t, ngapPdu.InitiatingMessage.ProcedureCode.Value, ngapType.ProcedureCodeDownlinkNASTransport, "Expected DownlinkNASTransport")
	//
...
func SingleRegistration(idx int, data MobileIdentityGroup, t *testing.T) (string, string, time.Duration, time.Duration) {
	...
	servingPlmnId := "208093" // 2-digit MNC requires leading zero
	test.InsertAuthSubscriptionToMongoDB(ue.Supi, ue.AuthenticationSubs)
	getData := test.GetAuthSubscriptionFromMongoDB(ue.Supi)
	assert.NotNil(t, getData)
	{
