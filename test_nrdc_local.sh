...
AMF_N2_IP=${AMF_N2_IP:-10.0.1.1}
UPF_N3_IP=${UPF_N3_IP:-$AMF_N2_IP}
DC_GNB_IP=${DC_GNB_IP:-10.0.1.3}
GNB_API_HOST_M=${GNB_API_HOST_M:-10.0.1.2}
GNB_API_HOST_S=${GNB_API_HOST_S:-10.0.1.3}
GNB_API_PORT=${GNB_API_PORT:-40104}
AMF_NGAP_PORT=${AMF_NGAP_PORT:-38412}
AMF_INSTANCE_ID=${AMF_INSTANCE_ID:-11}
AMF_ONVM_RESET_ON_CONFLICT=${AMF_ONVM_RESET_ON_CONFLICT:-1}
...
SKIP_NRDC_TRIGGER=${SKIP_NRDC_TRIGGER:-0}
# This test requires the ONVM-XIO control plane. Do NOT inherit these from the
# ambient shell: a stray `ONVMPOLLER_ENABLE=0` (e.g. left over from trying the
# TCP control-plane mode) makes each NF skip onvmpoller init, leaving
# local_address empty so the first SBI dial panics in inet_addr
# (NFs/xio/poller.go) -> "AMF did not bind SCTP". Force the working values and
# warn loudly if the environment tried to override them.
if [[ "${L25GC_LOCAL_CP_TCP:-0}" != "0" || "${ONVMPOLLER_ENABLE:-1}" != "1" ]]; then
    printf '\033[0;33m  overriding inherited CP env (L25GC_LOCAL_CP_TCP=%s, ONVMPOLLER_ENABLE=%s) -> forcing ONVM-XIO control plane\033[0m\n' \
        "${L25GC_LOCAL_CP_TCP:-unset}" "${ONVMPOLLER_ENABLE:-unset}"
fi
L25GC_LOCAL_CP_TCP=0
ONVMPOLLER_ENABLE=1
export L25GC_LOCAL_CP_TCP ONVMPOLLER_ENABLE
...
stage_local_configs() {
    ...
    awk -v amf_ip="$AMF_N2_IP" '
        ...
        in_ngap && $0 !~ /^[[:space:]]*$/ { in_ngap=0 }
        { print }
    ' "$BACKUP_DIR/amfcfg.yaml" > "$amf_tmp"

    python3 - "$BACKUP_DIR/smfcfg.yaml" "$smf_tmp" "$UPF_N3_IP" <<'PY'
import sys
src, dst, upf_ip = sys.argv[1:]
...
while i < len(lines):
    line = lines[i]
    out.append(line)
    if "interfaceType:" in line and "N3" in line:
        i += 1
        while i < len(lines):
            cur = lines[i]
            if cur.lstrip().startswith("endpoints:"):
                out.append(cur)
                i += 1
                if i < len(lines):
                    endpoint_line = lines[i]
                    indent = endpoint_line[:len(endpoint_line) - len(endpoint_line.lstrip())]
                    out.append(f"{indent}- {upf_ip}")
                    i += 1
                    while i < len(lines):
                        nxt = lines[i]
                        stripped = nxt.lstrip()
...
PY
    ...
    python3 - "$BACKUP_DIR/upfcfg.yaml" "$upfc_tmp" "$UPF_N3_IP" <<'PY'
import sys
src, dst, upf_ip = sys.argv[1:]
lines = open(src).read().splitlines()
out = []
...
for line in lines:
    ...
    if not patched and stripped.startswith("- addr:"):
        indent = line[:len(line) - len(stripped)]
        out.append(f"{indent}- addr: {upf_ip}")
        patched = True
    else:
        out.append(line)
open(dst, "w").write("\n".join(out) + "\n")
PY
    ...
    sed -E \
        ...
        -e 's/(dc_gnb_ip: ).*/\1"'"$DC_GNB_IP"'"/' \
        -e 's/(dc_upf_ip: ).*/\1"'"$UPF_N3_IP"'"/' \
        -e 's/(upf_ip: ).*/\1"'"$UPF_N3_IP"'"/' \
...
launch_nf_bg() {
    ...
    ( cd "$L25GC" && NF_NAME="$name" sudo -nE env \
        L25GC_LOCAL_CP_TCP="$L25GC_LOCAL_CP_TCP" \
        ONVMPOLLER_ENABLE="$ONVMPOLLER_ENABLE" \
...
amf_socket_seen() {
    local addr_pat="${AMF_N2_IP//./\\.}:${AMF_NGAP_PORT}"
    sudo ss -lpnA sctp 2>/dev/null | grep -Eq "$addr_pat|:${AMF_NGAP_PORT}" \
        || grep -q "Listen on ${AMF_N2_IP}:${AMF_NGAP_PORT}" "$LOG/amf.log" 2>/dev/null
}

amf_alive_and_listening() {
    amf_socket_seen && pgrep -x amf >/dev/null
}

amf_needs_onvm_reset() {
    grep -qE "Selected ID already in use|Failed ONVM init" "$LOG/amf.log" 2>/dev/null
}

restart_onvm_stack() {
    yel "  restarting ONVM manager + UPF pair due to manager-side NF state conflict"
    sudo pkill -9 -f 'onvm_mgr|l25gc_upf_u|l25gc_upf_c' 2>/dev/null || true
    sleep 2
    start_onvm_manager
    if ! pgrep -u root onvm_mgr >/dev/null; then
        return 1
...
# If the RAN source is present (submodule checked out) but not built yet, build
# it. Lets a fresh clone run this test without a separate manual `make bin`.
if [[ -d "$RAN" && -f "$RAN/Makefile" && ! -x "$RAN/build/free-ran-ue" ]]; then
    step "ran: building free-ran-ue (first run)"
    if ( cd "$RAN" && make bin ); then
        grn "  free-ran-ue built"
    else
        yel "  free-ran-ue build failed; will fall back to logic-only selftest"
    fi
fi
...
# Also tear down the ONVM manager + UPF secondaries. Reusing a stale onvm_mgr
# across runs leaves the shared-memory NF table in a bad state, which makes a
# fresh NF's onvmpoller init resolve an empty local_address and panic in
# inet_addr (NFs/xio/poller.go) -> AMF never binds N2 SCTP. Start each run with
# a fresh manager instead. (exact-name match so we never kill this script.)
sudo pkill -9 -x onvm_mgr 2>/dev/null || true
sudo pkill -9 -x l25gc_upf_u 2>/dev/null || true
sudo pkill -9 -x l25gc_upf_c 2>/dev/null || true
sudo rm -f "$LOG"/*.log
cleanup_local_dn_topology
sleep 2
# ---------------------------------------------------------------------------
...
for amf_attempt in $(seq 1 "$AMF_MAX_ATTEMPTS"); do
    ...
    if [[ $socket_seen -ne 1 ]]; then
        if [[ "$AMF_ONVM_RESET_ON_CONFLICT" == "1" ]] && amf_needs_onvm_reset; then
            yel "  AMF hit ONVM init conflict before SCTP bind; resetting manager/upf stack"
            restart_onvm_stack || true
        fi
    ...
    if [[ "$AMF_ONVM_RESET_ON_CONFLICT" == "1" ]] && amf_needs_onvm_reset; then
        yel "  AMF lost liveness after ONVM init conflict; resetting manager/upf stack"
        restart_onvm_stack || true
    fi
...
if [[ "$SKIP_NRDC_TRIGGER" == "1" ]]; then
    UE_IP=$(awk '/PDU session UE IP:/ {print $NF; exit}' "$LOG/ue.log" 2>/dev/null || true)
    if [[ -n "$UE_IP" ]]; then
        grn "  PDU session UE IP: $UE_IP"
    fi
    grn "PASS: local single-node stack is up through initial PDU session establishment."
    grn "      NR-DC trigger was skipped because SKIP_NRDC_TRIGGER=1."
    exit 0
fi
# ---------------------------------------------------------------------------
